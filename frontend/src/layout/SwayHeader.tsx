...
export default function SwayHeader() {
  ...
  return (
    <nav
      className="flex flex-wrap items-center gap-x-3 gap-y-4 border-b border-white/10 pb-5 sm:gap-x-4 sm:pb-6 md:flex-nowrap md:gap-x-6"
    ...
    >
      ...
      <ul className="order-last grid w-full grid-cols-3 gap-1 rounded-2xl border border-white/10 bg-white/5 p-1 md:order-none md:mx-auto md:flex md:w-auto md:shrink-0">
        {navigation.map((item) => {
          const isActive =
            item.to === "/dashboard"
      ...
      <div className="ml-auto flex max-w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2 md:ml-0">
        <Link
